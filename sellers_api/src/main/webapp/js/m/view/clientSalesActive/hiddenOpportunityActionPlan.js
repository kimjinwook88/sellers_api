var action = {
	 ...
	 */
	actAdd : function(map){
		$.get('/ajaxHtml/mobileActionPlan.html', function(data){
		    $('.cont2 ul').append(data);
		}).done(function(){
			if(map){
				var idx = $('.cont ul li').length-1;
				$("input[name='hiddenActId']").eq(idx).val(map.ACTION_ID);
				$("textarea[name='textareaActContents']").eq(idx).text(map.DETAIL_CONENTS);
				$("input[name='textActMember']").eq(idx).val(map.ACTION_OWNER);
				$("input[name='hiddenActMemberId']").eq(idx).val(map.ACTION_OWNER_ID);
				$("input[name='hiddenActMemberName']").eq(idx).val(map.ACTION_OWNER + (map.ACTION_OWNER_POSITION != null ? ' '+ map.ACTION_OWNER_POSITION : ''));	
	...
	actSubmitList : function(){
		...
		$('.cont2 li').each(function(idx,val){
			...
			if(($('textarea[name="textareaActContents"]').eq(idx).val()).trim()){ //내용을 작성한것만 담는다.
				actMap.DETAIL_CONENTS = $('textarea[name="textareaActContents"]').eq(idx).val();
				actMap.ACTION_OWNER_ID = $('input[name="hiddenActMemberId"]').eq(idx).val();
				actMap.ACTION_OWNER = $('input[name="hiddenActMemberName"]').eq(idx).val();
				...
				if(isEmpty(actMap.ACTION_OWNER_ID)){
					fncSelectTab(2);
...
$(document).ready(function() {
	...
	$('.cont2 ul').on('focus', 'input[name="textActMember"]', function(e){
		if($(this).attr('auto-bind') == '0'){
			commonSearch.member($(this), $(this).next($('input[name="hiddenActMemberId"]')));
