...
DayGrid.mixin({
	...
	computeRowLevelLimit: function(row) {
		...
		for (i = 0; i < trEls.length; i++) {
			...
			if (trEl.position().top + trHeight > rowHeight) {
				return i; // more count control 2017.05.23 wook
			}
		}
	...
	renderMoreLink: function(row, col, hiddenSegs) {
		...
		/*//moveTime remove
		var hiddenSegsLength = 0;
		for(var i=0; i<hiddenSegs.length; i++){
			if(hiddenSegs[i].event.EVENT_CODE != '5'){
				hiddenSegsLength ++;
			}
		}*/
