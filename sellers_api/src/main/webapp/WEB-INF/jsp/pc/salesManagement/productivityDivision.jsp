		...
		success : function(data){
			...
			for(var i=0; i<data.rows.length; i++){
				...
				if(data.rows[i].DIVISION_NAME !=null){
					...
					if(data.rows[i].DIVISION_NAME == "회사평균"){
						//data.rows[i].DIVISION_NAME = '회사평균';
						
						dataArr.push(data.rows[i].DIVISION_NAME, data.rows[i].avr);
					}
					else
					{
						//string 사업본부 제거
						var teamName_dataObjLabel= data.rows[i].TEAM_NAME.replace("사업본부", "");
						teamName_dataObjLabel = teamName_dataObjLabel.replace("사업부", "");
						teamName_dataObjLabel = teamName_dataObjLabel.replace("팀", "");
						
						dataArr.push(teamName_dataObjLabel, data.rows[i].avr);
					}
