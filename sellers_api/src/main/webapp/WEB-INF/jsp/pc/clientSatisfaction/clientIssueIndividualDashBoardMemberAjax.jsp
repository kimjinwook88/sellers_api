	...
	<c:when test="${fn:length(selectClientIssueDashBoardMember) > 0}">
		...
		<c:choose>
			...
			<c:otherwise>
			    ...
			    <th><a href="javascript:void(0);" onClick="dashboard.goList('${member.MEMBER_DIVISION}','${member.MEMBER_TEAM}','${member.MEMBER_ID_NUM}', null, null);"> ${member.HAN_NAME}</a></th>
			    <td class="ag_c"><fmt:formatNumber value="${member.TOTAL_COUNT}" groupingUsed="true"/></td>
			    <td class="ag_c">
							<a href="javascript:void(0);" onClick="if(dashboard.countCheck(this))dashboard.goList('${member.MEMBER_DIVISION}','${member.MEMBER_TEAM}','${member.MEMBER_ID_NUM}','제품', null);">
								<fmt:formatNumber value="${member.CATEGORY_1}" groupingUsed="true"/>
							</a>
						</td>
						<td class="ag_c">
							<a href="javascript:void(0);" onClick="if(dashboard.countCheck(this))dashboard.goList('${member.MEMBER_DIVISION}','${member.MEMBER_TEAM}','${member.MEMBER_ID_NUM}','서비스', null);">
								<fmt:formatNumber value="${member.CATEGORY_2}" groupingUsed="true"/>
							</a>
						</td>
						<td class="ag_c">
							<a href="javascript:void(0);" onClick="if(dashboard.countCheck(this))dashboard.goList('${member.MEMBER_DIVISION}','${member.MEMBER_TEAM}','${member.MEMBER_ID_NUM}','지원', null);">
