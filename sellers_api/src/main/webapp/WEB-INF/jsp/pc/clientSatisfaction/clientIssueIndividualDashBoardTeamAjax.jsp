	...
	<c:when test="${fn:length(selectClientIssueDashBoardTeam) > 0}">
		...
		<c:choose>
			<c:when test="${fn:contains(auth, 'ROLE_TEAM')}">
				<tr class="tr_list" id="team_${team.MEMBER_TEAM}" data-division="${team.MEMBER_DIVISION}" name="sub_tr_${team.MEMBER_DIVISION}">	
			</c:when>
			<c:otherwise>
				<tr class="tr_list depth2" id="team_${team.MEMBER_TEAM}" data-division="${team.MEMBER_DIVISION}" name="sub_tr_${team.MEMBER_DIVISION}">
			</c:otherwise>
		</c:choose>
			    ...
			    <td class="ag_c">
							<a href="javascript:void(0);" onClick="if(dashboard.countCheck(this))dashboard.goList('${team.MEMBER_DIVISION}','${team.MEMBER_TEAM}',null,'제품');">
								<fmt:formatNumber value="${team.CATEGORY_1}" groupingUsed="true"/>
							</a>
						</td>
						<td class="ag_c">
							<a href="javascript:void(0);" onClick="if(dashboard.countCheck(this))dashboard.goList('${team.MEMBER_DIVISION}','${team.MEMBER_TEAM}',null,'서비스');">
								<fmt:formatNumber value="${team.CATEGORY_2}" groupingUsed="true"/>
							</a>
						</td>
						<td class="ag_c">
							<a href="javascript:void(0);" onClick="if(dashboard.countCheck(this))dashboard.goList('${team.MEMBER_DIVISION}','${team.MEMBER_TEAM}',null,'지원');">
