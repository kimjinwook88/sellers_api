	...
	function fuaAdd(map){
		$.get("/ajaxHtml/followUpAction.html", function(data){
			...
			if(map){
				...
				$("textarea[name='textareaFuaContents']").eq(idx).text(map.SOLVE_PLAN);
	...
	function fuaSubmitList(){
		...
		$('tbody#tbodyFua tr').each(function(idx,val){
			...
			if(($("textarea[name='textareaFuaContents']").eq(idx).val()).trim()){ //내용을 작성한것만 담는다.
				fuaMap.SOLVE_PLAN = $("textarea[name='textareaFuaContents']").eq(idx).val();
				fuaMap.SOLVE_OWNER_ID = $("input[name='hiddenFuaMemberId']").eq(idx).val();
				fuaMap.SOLVE_OWNER = $("input[name='hiddenFuaMemberName']").eq(idx).val();
...
<p class="text-center pd-t10">
	<a href="javascript:void(0);" class="btn-row-add" name="buttonSaleAddRow" onclick="fuaAdd();">+ 이슈해결계획 추가</a>
</p>