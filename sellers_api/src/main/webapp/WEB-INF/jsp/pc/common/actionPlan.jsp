	...
	function fuaAdd(map){
		$.get("/ajaxHtml/followUpAction.html", function(data){
			...
			if(map){
				...
				$("textarea[name='textareaFuaContents']").eq(idx).text(map.DETAIL_CONENTS);
				$("input[name='textModalFuaPlanDate']").eq(idx).val(map.DUE_DATE);
				$("input[name='textModalFuaActualDate']").eq(idx).val(map.CLOSE_DATE);
				...
				$("input[name='hiddenFuaMemberId']").eq(idx).val(map.ACTION_OWNER_ID);
				$("input[name='hiddenFuaMemberName']").eq(idx).val(map.ACTION_OWNER + (map.ACTION_OWNER_POSITION != null ? ' '+ map.ACTION_OWNER_POSITION : ''));
				$("li[name='liFuaMemberSeach']").eq(idx).before(
						'<li class="value">'+
						'<span class="txt" id="'+ map.ACTION_OWNER_ID +'">'+ map.ACTION_OWNER +' ['+ map.ACTION_OWNER_POSITION +']</span>'+
	...
	function fuaSubmitList(){
		...
		$('tbody#tbodyFua tr').each(function(idx,val){
			...
			if(($("textarea[name='textareaFuaContents']").eq(idx).val()).trim()){ //내용을 작성한것만 담는다.
				fuaMap.DETAIL_CONENTS = $("textarea[name='textareaFuaContents']").eq(idx).val();
				fuaMap.ACTION_OWNER_ID = $("input[name='hiddenFuaMemberId']").eq(idx).val();
				fuaMap.ACTION_OWNER_NAME = $("input[name='hiddenFuaMemberName']").eq(idx).val();
				fuaMap.DUE_DATE = $("input[name='textModalFuaPlanDate']").eq(idx).val();
				if(isEmpty(fuaMap.ACTION_OWNER_ID)){
					$("#fuaTable").after('<label id="fuaTable-error" class="error-custom" for="fuaTable">담당자를 입력해 주세요.</label>'	);
					fuaValid = false;
					return false; 
				}
				if(isEmpty(fuaMap.DUE_DATE)){
					$("#fuaTable").after('<label id="fuaTable-error" class="error-custom" for="fuaTable">해결목표일을 입력해 주세요.</label>'	);
					fuaValid = false;
					return false; 
				}
				fuaMap.CLOSE_DATE = $("input[name='textModalFuaActualDate']").eq(idx).val();
...
<table id="fuaTable" class="table table-bordered table-inner"
	style="width: 680px; margin-bottom: 5px;">
	<colgroup>
		<col width="*">
		<col width="135px;">
		<col width="120px;">
...
<p class="text-center pd-t10">
	<a href="javascript:void(0);" class="btn-row-add"
		name="buttonSaleAddRow" onclick="fuaAdd();">+ Action Plan 추가</a>
</p>
