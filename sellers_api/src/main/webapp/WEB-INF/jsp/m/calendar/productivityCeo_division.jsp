				...
				<select class="form-control m-b" name="selectFaceTeam" id="selectFaceTeam">
					<c:choose>
						<c:when test="${fn:length(searchDetailGroup.faceTeam) > 0}">
							<option value="" <c:if test="${param.selectFaceTeam == null}"> selected='selected' </c:if> >전체 팀</option>
...
$(document).ready(function() {
	...
	function drawChart(){
		...
		var params = $.param({
			...
			<c:if test="${param.selectFaceTeam != null}">
				selectFaceTeam : teamName,
			</c:if>
			<c:if test="${param.selectFaceTeam == null}">
				selectFacePost: $ ("#selectFaceTeam").val(),
			</c:if>
			/*
			if(teamName != null{
				selectFaceTeam : teamName;
